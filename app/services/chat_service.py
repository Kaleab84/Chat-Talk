...
class ChatService:
                ...
                try:
                    ...
                    if getattr(settings, "OPENAI_API_KEY", None):
                        ...
                        try:
                            ...
                            logger.warning(
                                f"OpenAI generation also failed, using stub: {gpt_exc}"
                            )

                            answer = self._generate_simple_answer(
                                question, context_chunks, formatted_context
                            )
    ...
    def ask_question_with_images(
        self, question: str, images: List[Dict[str, Any]], top_k: int = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if getattr(settings, "GEMINI_API_KEY", None):
                ...
                try:
                    ...
                    answer = self._generate_gemini_answer_with_images(
                        question, formatted_context, images
                    )
    ...
    def _build_response_preamble(self, include_image_hint: bool = False) -> str:
        """Create a warm, shared system prompt for all model providers."""
        ...
        if include_image_hint:
            ...
            sentences.append(
                "Treat the attached images as part of the user's question and incorporate them when relevant."
            )
    ...
    def _generate_gpt_answer(self, question: str, formatted_context: str) -> str:
        ...
        system_prompt = self._build_response_preamble()
    ...
    def _generate_gemini_answer_with_images(
        self, question: str, formatted_context: str, images: List[Dict[str, Any]]
    ) -> str:
        ...
        system_prompt = self._build_response_preamble(include_image_hint=True)
    ...
    def get_recommendations(
        self, query: str, content_type: str = "all"
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            recommendations["documents"] = self._deduplicate_recommendations(
                recommendations["documents"]
            )

            recommendations["videos"] = self._deduplicate_recommendations(
            ...
            return {
                ...
                "total_items": len(recommendations["documents"])
                + len(recommendations["videos"]),
            }
    ...
    def _is_vector_store_empty(self) -> bool:
        ...
        try:
            ...
            logger.warning(
                "VectorStore is missing get_index_stats(); skipping empty-store check."
            )
